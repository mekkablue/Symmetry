...
class Symmetry ( NSObject, GlyphsReporterProtocol ):
	...
	def drawBackgroundForLayer_( self, Layer ):
		...
		"""
		try:
			NSColor.colorWithCalibratedRed_green_blue_alpha_( 1.0, 1.0, 0.0, 0.4 ).set() # sets RGBA values between 0.0 and 1.0

			bezierPath = Layer.bezierPath()
			xShift = Layer.bounds.origin.x * 2 + Layer.bounds.size.width
			
			mirroring = NSAffineTransform.transform()
			mirroring.scaleXBy_yBy_( -1.0, 1.0 )
			mirroring.translateXBy_yBy_( -xShift, 0.0 )
